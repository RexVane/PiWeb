...
A version that already exists on npm is bumped past, so re-runs and concurrent pushes never fail on a duplicate. The workflow skips its own release commits. Nothing is published when the gate fails.
...
One-time setup: create an npm **Automation token** (npmjs.com → Access Tokens) and store it as the repository secret `NPM_TOKEN`; without it the publish step fails with an authentication error. To skip a release for one push (for example a docs-only commit), include `[skip ci]` in the commit message.

Users update whenever they choose:
