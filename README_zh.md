...
审查后的代码修复已落实在当前**未提交工作树**：覆盖登录页生产构建、经过 Next.js 代理的上传完整性、扩展运行时按会话隔离与重载后的工具权限、模型及设置的无损写入、输入草稿与扩展待答界面的生命周期、Git/Growth/diff 正确性，以及隔离发布构建。隔离生产集成验证通过了 11 MiB 上传、扩展会话隔离、只读权限重载、待答界面断线恢复、提示词接收确认与后续队列。

最近一次本地验证通过 TypeScript 检查、**52 个测试文件中 355 项通过、1 项跳过**、`npm run build` 和 `git diff --check`。跳过项需要 Windows 符号链接权限或开发者模式；构建仍有一条来自 `scripts/release.mjs` 动态依赖的非阻断 webpack 警告。

这不等于真实部署验收：未执行真实 OAuth、外部更新/安装，也未重启现有 PiWeb 服务。隔离发布检查已经通过，但现有进程仍使用原构建，需在维护窗口手动重启后才会切换。
