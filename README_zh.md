...
点击输入框左下角的 **+**，可按会话选择 **Agent / Plan / Goal**。Agent 沿用原有工具预设直接执行；Plan 只能使用只读工具，给出方案后停下，必须点击「执行方案」才会临时开放当前工具预设执行，完成后恢复只读；Goal 将首条消息固定为当前目标，在后续回合持续提醒 Agent 验证结果，但不会无限自动发起新回合。模式状态写入原生 pi 会话的自定义记录，重新打开会话仍可恢复。

Plan 的工具限制不是对本地可信 pi 扩展代码的安全沙箱。审批绑定最近一次方案；方案之后若对话内容变化，需要重新生成方案。

点击已发送的用户消息即可原位编辑并重发，文字不变也能重新运行。PiWeb 会从该消息重新生成当前对话，而不是在底部追加新消息；旧分支仍保留在原生会话历史中。

标准或完整工具预设下，Agent 可调用 `piweb_delegate`，一次并行委派最多 3 个独立研究任务。子代理使用当前模型、只读文件工具和临时内存会话，不能运行命令、改文件或继续委派；报告汇总回主会话，不产生独立 Worker 会话。显式选择「只读」工具预设时不会自动启用委派。
...
在干净且已信任的 Git 仓库里，一次启动 1 到 3 个写明的任务。每个任务在独立 Git 工作树里执行，只有文件工具，不能跑 shell、扩展或自动测试。结束后保留报告和补丁供审查。采纳只改主工作区里的文件，不暂存、也不提交。

### 测试检查器

列出当前工作区 `tests/` 下的测试文件，打开一份，并用项目自己的 Vitest 跑这一份。页面显示通过、失败、跳过和命令输出，不编造覆盖率或内存数字。项目信任不是操作系统沙盒。
...
也可以装指定版本：`npm install -g @rexvane/piweb@0.3.16`。
