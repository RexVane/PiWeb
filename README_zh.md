...
全局安装会在安装时或首次运行时准备一次生产产物；失败时会明确报错而不是回退开发服务器，可用 `npm rebuild -g piweb` 重试。**已知问题**：全局安装包内的一次性构建尚未验证通过，暂时建议优先使用下面的仓库方式。安装同时注册 `pi` 与 `piweb` 两个命令（`pi web` 启动界面，其它参数转发给官方 pi CLI）；如果同时全局安装了官方 `pi` 包，两者会争用同一个 `pi` 命令，最后安装的生效。
